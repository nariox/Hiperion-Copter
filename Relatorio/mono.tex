...
\begin{equation}
  \ddot{\a} = - \frac{1}{J_\phi} (F_{M_1} + F_{M_2} - F_{M_3} - F_{M_4}) d.
\end{equation}
...
A partir das três tabelas criadas foram obtidas todas as curvas. A primeira delas é ---.
...
A tabela \ref{tabmotor} foi a responsável por gerar as curvas ------ ------ -----, de modo que as conclusões possam ser mais fáceis percebidas. Com base nesta foi possível comparar dois modelos de hélices diferentes e também seus principais parâmetros como a rotação, a tensão, a força e a corrente. 
...
A partir do gráfico ------- e também da tabela \ref{tabmotortensao} pode ser observado a influência da variação da tensão na força e na corrente do motor.
