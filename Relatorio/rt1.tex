...
%\comentario{Proposta de Projeto Final apresentado na disciplina PSI-2591 Projeto de Formatura I.}
...
O tema sobre controle de multirrotores voltou a ser estudado, graças a tecnologias novas e o grande interesse em \textit{VANTs}. Há diversas pesquisas já publicadas e os métodos desenvolvidos obtem resultados satisfatório, apesar de não haver o estabelecimento de uma teoria clássica especificamente para multirrotores. Abaixo são relatados alguns trabalhos desenvolvidos com esse tema que propõem diferentes abordagens para essa tarefa.

\citeonline{modellingpid} apresenta um modelo para um quadrirotor e é desenvolvido um método de controle proporcional-integral-derivativo (PID) clássico, com quatro forças de entrada que são os empuxos gerados pelos quatro rotores.

Em \cite{nlpid} é projetado um controlador não linear baseado no esquema clássico de controle proporcional-integral-derivativo (PID) para controlar um quadrrotor com seis graus de liberdade. A posição horizontal do quadrirotor é controlada por ações PI, enquanto a orientação e a posição vertical são controladas por algorítmos PID. A performance é verificada por testes numéricos, embora apenas os efeitos da força gravitacional sejam compensados.

%\cite{rbfarx} sugere um modelo MIMO RBF-ARX para representar a dinâmica não-linear de um quadrirrotor e, baseando-se em um modelo ARX linearizado obtido do RBF-ARX no ponto de operação, é proposto o controle por realimentação de estado com abordagem LQR. Por fim, a proposta é aplicada num quadriotor, confirmando a validade do modelo RBF-ARX.

\citeonline{visionuav} revê o modelo matemático de um quadrirotor usando a equação de Lagrange. Então é proposto um método de controle linear baseado em imagens de duas câmeras, uma no quadrrotor e outro no solo.
...
\chapter{Arquitetura}
A arquitetura proposta foi pensada de forma a permitir ao multirrotor fácil adaptabilidade a projetos de pesquisa futuros e a outras finalidades não previstas ou não implementadas no protótipo inicial. Por isso, o software e o hardware foram divididos em quatro camadas de abstração, cada qual com uma parcela bem definida das tarefas que, em conjunto, são responsáveis pelo funcionamento do sistema.
...
Essas camadas ou domínios de tarefas são:
...
\begin{enumerate}
  ...
  \item     Camada de navegação, responsável pela velocidade e pela direção do sistema;
...
A figura \ref{fig.arquitsimp} dá uma visão geral da arquitetura, que será detalhada mais adiante.
\begin{figure}[h!]
  \centering
  \includegraphics[width=4in]{./Figuras/Arquitsimp.eps}
...
É constituída pelos n motores, hélices e controladores eletrônicos de velocidade (eletronic speed controllers - ESCs), sendo n o número de rotores do multirrotor. Os ESCs, por sua vez, são constituídos por duas placas de circuito, sendo uma responsável pela lógica e a outra pelo fornecimento de potência aos motores.

As hélices são presas nos eixos dos motores de forma que girem com ele. Cada motor é ligado à placa do circuito de potência do ESC correspondente, pela qual é alimentado. A placa de potência do ESC é ligada à sua placa de lógica e à bateria, a qual fornece a tensão e a corrente necessárias para o acionamento dos motores.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=4in]{./Figuras/Excit-func.eps}
...
Camada constituída por um microcontrolador ARM (Cortex-M0 modelo LPC1114 da NXP, família ARMv6-M, arquitetura Cortex-M) e por uma unidade de medida inercial (IMU - inertial measurement unit), que é um circuito que contém o acelerômetros e o giroscópio tridimensionais. O microcontrolador está ligado ao microcontrolador da camada de navegação por SPI e à  IMU por $I^2C$.
Parâmetros de entrada e saída

Entrada:

    da camada de navegação
        ângulos de Euler e potência total dos motores (parâmetros de navegação)

Saídas:

    para a camada de excitação dos motores
        rotação de cada motor
    para a camada de navegação
        status dos motores
        dados dos sensores de estabilização - acelerômetro e giroscópio
...
Os algoritmos que controlam a estabilidade são PIDs não-lineares, ver o capítulo ``Controle'' para mais detalhes.
...
Entradas:
    ...
    da camada de periféricos
        sinais dos sensores de navegação (GPS e magnetômetro)
        parâmetros de direção, velocidade e rotação do multirrotor
    da camada de estabilização de vôo
        status dos motores
        dados dos sensores de estabilização (acelerômetro e giroscópio)

Saída:

    para a camada de estabilização de vôo
        ângulos de Euler e potência total dos motores (parâmetros de navegação)
    para a camada de periféricos
        status dos motores (caso solicitado por algum módulo periférico)
        dados dos sensores de estabilização - acelerômetro e giroscópio (caso solicitado por algum módulo periférico)
...
Em seguida ocorre a leitura dos sinais dos sensores de navegação (GPS e magnetômetro). A partir desses parâmetros e sinais é realizado o processamento que gera os parâmetros de navegação (ângulos de Euler e potência total dos motores) que são enviados para o microcontrolador da camada de estabilização.

Abaixo, um diagrama que representa esse comportamento:

Figura 4: comportamento da camada de navegação
Camada de periféricos
Descrição

A camada de periféricos abriga originalmente os módulos dos sensores de navegação e o módulo receptor do rádio-controle, que estão ligados entre si e ao microcontrolador da camada de navegação por um barramento I²C. Essa arquitetura foi concebida para que seja possível que qualquer pessoa com o conhecimento técnico necessário crie e adicione periféricos personalizados.
...
Camada expansível constituída por módulos capazes de se comunicar por I²C. Os módulos implementados originalmente são os sensores de navegação, que contém o GPS e o magnetômetro, e o módulo receptor de rádio-controle.
Parâmetros de entrada e saída

Entrada:

    sinal do rádio-controle
